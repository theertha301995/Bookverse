...
export const updateReadingHistory = async (req, res) => {
    try {
        ...
        const history = await ReadingHistory.findOneAndUpdate({ userId, bookId }, { chapterId, progress }, { upsert: true, new: true });
        res.json(history);
    }
    catch (err) {
...
export const getLastReadChapter = async (req, res) => {
    try {
        const userId = req.user?.id;
...
//# sourceMappingURL=readingHistory.controller.js.map