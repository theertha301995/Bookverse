...
export const getBookSummary = async (bookId: string) => {
  // 1. Check cache
  ...
  if (!fullText) throw new Error("No content available.");
...
export const getChapterSummary = async (chapterId: string) => {
  // 1. Check cache
  ...
  if (!text) throw new Error("Chapter is empty.");
